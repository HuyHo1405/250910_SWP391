...
@Tag(name = "Authentication", description = "Endpoints for user authentication and authorization - Guest, Authorized User")
public class AuthController {
    private final IAuthService authService;

    @PostMapping("/register")
    @Operation(summary = "Register a new user", description = "Allows a guest to register a new account.")
    public ResponseEntity<MessageResponse> register(@RequestBody @Valid AuthRequest.Register request) {
        return ResponseEntity.ok(authService.register(request));
    }

    @PostMapping("/login")
    @Operation(summary = "Login to the system", description = "Allows a guest to login and receive authentication tokens.")
    public ResponseEntity<AuthResponse> login(@RequestBody @Valid AuthRequest.Login request) {
        return ResponseEntity.ok(authService.login(request.getUserName(), request.getPassword()));
    }

    @PostMapping("/verify")
    @Operation(summary = "Verify email address", description = "Verifies a user's email address using a verification code.")
    public ResponseEntity<MessageResponse> verifyEmail(@RequestBody @Valid AuthRequest.Verify request) {
        return ResponseEntity.ok(authService.verifyEmail(request));
    }

    @PostMapping("/resend-verification-code")
    @Operation(summary = "Resend verification code", description = "Resends the email verification code to the user's email address.")
    ...
    @PostMapping("/logout")
    @Operation(summary = "Logout from the system", description = "Logs out the currently authenticated user.")
    public ResponseEntity<MessageResponse> logout(@RequestBody @Valid AuthRequest.Logout request) {
        return ResponseEntity.ok(authService.logout(request));
    }

    @PostMapping("/forgot-password")
    @Operation(summary = "Forgot password", description = "Initiates the password reset process for a user who forgot their password.")
    ...
    @PostMapping("/refresh")
    @Operation(summary = "Refresh authentication token", description = "Refreshes the authentication token for a logged-in user.")
